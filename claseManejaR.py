...
class ManejaRegistro:
    def __init__(self):
        self.__Registro =[[None for columna in range(3)]for fila in range(24)]
    def cargararchivo (self):
        archivo=('Registro_ej3.csv')
        for fila in range(24):
            for columna in range(3):
                print ("Ingresar datos del día "+str(fila+1))
                print ("Ingresar datos de la hora "+ str(datetime.time(columna)))
                temp=float(input("Ingresar la temperatura actual "))
                humed=float(input("Ingresar la humedad actual "))
                pres=float(input("Ingresar la presión actual "))
                registroaux=Registro(temp,humed,pres)
                self.__Registro[fila][columna]=registroaux
                os.system('cls')
            i=0
        os.system('cls')
        with open (archivo, 'w', newline='') as archivo_csv:
                escritor= csv.writer(archivo_csv, delimiter=';')
                for fila in self.__Registro:
                    j=0
                    for columna in fila:
                        escritor.writerow([i+1,datetime.time(j),columna.getTemperatura(),columna.getHumedad(),columna.getPresion()])
                        j+=1
                    i+=1
